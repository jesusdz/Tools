...
echo "$BIN_DIR/lldb-server platform --server --listen $LISTENER_SCHEME://$DOMAINSOCKET_DIR/$PLATFORM_SOCKET --log-file \"$PLATFORM_LOG_FILE\" --log-channels \"$LOG_CHANNELS\" </dev/null >$LOG_DIR/platform-stdout.log 2>&1"
